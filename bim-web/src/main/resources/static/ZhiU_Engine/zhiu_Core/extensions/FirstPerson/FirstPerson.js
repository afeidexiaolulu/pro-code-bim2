...
ZhiUTechNamespace('ZhiUTech.Viewing.Extensions.FirstPerson');

    /**
     * First Person navigation tool, similar to those found in videogames.
     *
     * It will also replace the default walk tool button when {@link ZhiUTech.Viewing.GuiViewer3D} is present.
     * @constructor
     * @extends {ZhiUTech.Viewing.Extension}
     * @param {ZhiUTech.Viewing.Viewer3D} viewer - Viewer instance.
     * @param {object} options - Not used.
     * @category Extensions
     */
ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension = function(viewer, options) {
    ZhiUTech.Viewing.Extension.call(this, viewer, options);
};

ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension.prototype = Object.create(ZhiUTech.Viewing.Extension.prototype);
ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension.prototype.constructor = ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension;

ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension.prototype.load = function() {
    var self = this;
    var viewer = this.viewer;
    var avu = ZhiUTech.Viewing.UI;

    // Register tool
    this.tool = new ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonTool(viewer);
    ...
    viewer.addEventListener(ZhiUTech.Viewing.TOOL_CHANGE_EVENT, this.onToolChanged);

    return true;
};

ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension.prototype.createUI = function()
{
    var viewer = this.viewer;
    ...
    this.firstPersonToolButton.setIcon("zu-icon-first-person");
...
ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension.prototype.unload = function()
{
    var viewer = this.viewer;

    // Remove listeners
    viewer.removeEventListener(ZhiUTech.Viewing.TOOL_CHANGE_EVENT, this.onToolChanged);
    ...
    if (this.firstPersonToolButton) {
        ...
        if (toolbar) {
            toolbar.getControl(ZhiUTech.Viewing.TOOLBAR.NAVTOOLSID).removeControl(this.firstPersonToolButton.getId());
        }
...
ZhiUTech.Viewing.theExtensionManager.registerExtension('ZhiUTech.FirstPerson', ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonExtension);
...
(function() {
    ...
    ZhiUTech.Viewing.Extensions.FirstPerson.FirstPersonTool = function ( viewerapi ) {
        ...
        var _keys = ZhiUTech.Viewing.theHotkeyManager.KEYCODES;
        ...
        if(navigator.getGamepads || !!navigator.webkitGetGamepads || !!navigator.webkitGamepads){
            _gamepadModule = new ZhiUTech.Viewing.Extensions.GamepadModule(viewerapi);
        }
        ...
        var firstPersonToolUsed = function () {
            if(!_trackToolUsed) {
                zvp.logger.track({category: 'tool_used', name: 'FirstPerson_tool'});
                _trackToolUsed = true;
            }
        }
        ...
        var showHUD = function(messageSpecs, showDelay, closeCB, buttonCB, checkboxCB)
        {
            ...
            if (!ZhiUTech.Viewing.Private.HudMessage) {
                return;
            }
        ...
        addCrosshair = function() {
            if (zvp.stringToDOM) {
                this.crosshair = zvp.stringToDOM('<div id="remote-crosshair"><div class="crosshair-v"></div><div class="crosshair-h"></div></div>f');
