...
(function() {
    ...
    MeasureExtension.prototype.markupactivate = function (mode) {
        this.enterMarkupMode();
    ...
    /*  markup insetbutton start  */
    MeasureExtension.prototype.enterMarkupMode = function() {
//lfx
//     if (this._measurementMode) return;
//     this._measurementMode = true;
        ...
        this.markupControls = toolbar.getControl(ZhiUTech.Viewing.TOOLBAR.MARKUPTOOLSID);
        this.markupControls.setVisible(true);
        this.markupControls.container.style.display = '';

    };
    /*  markup insetbutton end  */
    /**
     * @private
     */
    MeasureExtension.prototype.exitMeasurementMode = function() {
        ...
        document.getElementById('markupTools').style.display = 'none'
    };
    MeasureExtension.prototype.exitMarkupmentMode = function(markup) {
        markup.setVisible(false);
        var toolbar = this.viewer.getToolbar(false);
        var viewerToolbarContainer = toolbar.container;
        var viewerContainerChildrenCount = viewerToolbarContainer.children.length;
    ...
    MeasureExtension.prototype.createUI = function()
    {
        ...
        this.markupmentToolbarButton = new avu.Button('toolbar-markupmentSubmenuTool');
        this.markupmentToolbarButton.setToolTip('Markup');
        this.markupmentToolbarButton.setIcon("icon-zhibiaozhushibiaozhu");
        modelTools.markupmentToolbarButton = this.markupmentToolbarButton;
        modelTools.addControl(this.markupmentToolbarButton, {index:0});
        this.markupmentToolbarButton.onClick = this.markupactivate.bind(this);

        var regionDiv = document.createElement("div");
        var regionDivBig = document.createElement("div");
        var regionclose = document.createElement("div");
        var regiontext = document.createElement("div");
        var buttonV = document.createElement("div");
        var buttonN = document.createElement("div");
        var regionsure = document.createElement("div");
        var regioncancel = document.createElement("div");
        buttonV.innerHTML ='<input type="checkbox" id="checkV-me" ><label for="checkV-me" class="label"><svg viewBox="-9 -9 43 35.191" width="43" height="35"><path id="check-icon" d=" M 0 9 Q 8 17 8 17 Q 8 17 25 0" fill="none" stroke-width="6" stroke="rgb(0,0,0)" stroke-linecap="round" /></svg></label><span>选出体积大于:</span><input type="text" disabled="true"  id="btnVvalue" placeholder="0" />的构件<p id="btnVtip">未输入筛选体积</p>'
        buttonN.innerHTML ='<input type="checkbox" id="checkN-me" ><label for="checkN-me" class="label"><svg viewBox="-9 -9 43 35.191" width="43" height="35"><path id="check-icon" d=" M 0 9 Q 8 17 8 17 Q 8 17 25 0" fill="none" stroke-width="6" stroke="rgb(0,0,0)" stroke-linecap="round" /></svg></label><span>需要忽略的构件:</span><input type="text" disabled="true"  id="btnNvalue" placeholder="输入构件名称" /><p id="btnNtip">未输入筛选名称</p>'
        ...
        regionclose.setAttribute("class", "docking-panel-close")
        ...
        viewer.clientContainer.appendChild(regionDivBig);
        this.regionmentToolbarButton = new avu.Button('toolbar-regionmentSubmenuTool');
        this.regionmentToolbarButton.setToolTip('Region');
        this.regionmentToolbarButton.setIcon("icon-juxing");
        modelTools.regionmentToolbarButton = this.regionmentToolbarButton;
        modelTools.addControl(this.regionmentToolbarButton, {index:0});
        $("#checkV-me").change(function(){
          if($("#btnVvalue").prop("disabled")){
              $("#btnVvalue").removeAttr("disabled");
          }else{
              $("#btnVvalue").attr("disabled",true);
          }
        });
        $("#checkN-me").change(function(){
            if($("#btnNvalue").prop("disabled")){
                $("#btnNvalue").removeAttr("disabled");
            }else{
                $("#btnNvalue").attr("disabled",true);
            }
        });
        $("#btnVvalue,#btnNvalue").blur(function(){
            $("#btnVtip").hide()
            $("#btnNtip").hide()
        });

        this.regionmentToolbarButton.onClick = function () {

            window.regionDivBig =regionDivBig
            window.regionDiv =regionDiv
            APImodel.esdstartSelection()

//差一个onchange  取消  案例
            regionclose.onclick = function () {
                window.regionDiv.style.display = 'none';
                window.regionDivBig.style.display = 'none';
                document.getElementById("btnVvalue").value = '';
                document.getElementById("btnNvalue").value = '';
                $("#btnNvalue").attr("disabled",true);
                $("#btnVvalue").attr("disabled",true);
                document.getElementById("checkV-me").checked =false
                document.getElementById("checkN-me").checked =false
                $("#btnVtip").hide()
                $("#btnNtip").hide()
            }
            regionsure.onclick=function () {
                var IsVchecked = document.getElementById("checkV-me").checked
                var IsNchecked = document.getElementById("checkN-me").checked
                var elementV = document.getElementById("btnVvalue").value;
                var elementN = document.getElementById("btnNvalue").value;
                if(IsVchecked ==true){
                    if(elementV==''){
                        $("#btnVtip").show()
                    }else{
                        elementV=Number(elementV)
                        APImodel._esdVolumeFilter(elementV,function (back) {
                            //console.log(back)
                        },true)
                        window.regionDiv.style.display = 'none';
                        window.regionDivBig.style.display = 'none';
                ...
                if(IsNchecked ==true){
                    ...
                    if(elementN==''){
                        ...
                        APImodel._esdNameFilter(elementN,function (back) {
                            // console.log(back)
                        },true)
                        window.regionDiv.style.display = 'none';
                        window.regionDivBig.style.display = 'none';
                ...
                $("#btnVtip").hide()
                $("#btnNtip").hide()
                if(IsNchecked ==false &&IsVchecked ==false){
                    $("#btnVtip").show()
                    $("#btnNtip").show()
                }
            }
        ...
        myclose.setAttribute("class", "docking-panel-close")
        volumeDiv.appendChild(myclose);
        volumeDiv.appendChild(mytext);

        window.viewer.clientContainer.appendChild(volumeDiv);

        this.volumeToolbarButton = new avu.Button('toolbar-volumeSubmenuTool');
        this.volumeToolbarButton.setToolTip('Volume');
        this.volumeToolbarButton.setIcon("icon-v1");
        modelTools.volumeToolbarButton = this.volumeToolbarButton;
        modelTools.addControl(this.volumeToolbarButton, {index:0});
        this.volumeToolbarButton.onClick = function () {
            APImodel.esdGetVolume()
            window.volumeDiv =volumeDiv
            window.mytext =mytext



            myclose.onclick = function () {
                window.volumeDiv.style.display = 'none';
            }
        }
      //gis
        this.gisToolbarButton = new avu.Button('toolbar-gisSubmenuTool');
        this.gisToolbarButton.setToolTip('Gis');
        this.gisToolbarButton.setIcon("icon-logistic");
        modelTools.gisToolbarButton = this.gisToolbarButton;
        modelTools.addControl(this.gisToolbarButton, {index:0});
        var num = 1
        this.gisToolbarButton.onClick = function () {
            if(num%2 ==1){
                window.viewer.showModel(2)
                document.getElementById("toolbar-gisSubmenuTool").classList.add("active")
            }else{
                window.viewer.hideModel(2)
                document.getElementById("toolbar-gisSubmenuTool").classList.remove("active")
            }
            num++
        }
        //gisend
        //workarea
/*        this.workareaToolbarButton = new avu.Button('toolbar-workareaSubmenuTool');
        this.workareaToolbarButton.setToolTip('Workarea');
        this.workareaToolbarButton.setIcon("icon-ccgl-yundandayinquyu-4");
        modelTools.workareaToolbarButton = this.workareaToolbarButton;
        modelTools.addControl(this.workareaToolbarButton, {index:0});
        var num1 = 1
        this.workareaToolbarButton.onClick = function () {
            if(num1%2 ==1){
                APImodel.esdWorkArea()
                document.getElementById("toolbar-workareaSubmenuTool").classList.add("active")
            }else{
                document.getElementById("toolbar-workareaSubmenuTool").classList.remove("active")
            }
            num1++
        }*/
        //end
        // milestone
        this.milestoneToolbarButton = new avu.Button('toolbar-milestoneSubmenuTool');
        this.milestoneToolbarButton.setToolTip('Milestone');
        this.milestoneToolbarButton.setIcon("icon-lichengbei");
        modelTools.milestoneToolbarButton = this.milestoneToolbarButton;
        modelTools.addControl(this.milestoneToolbarButton, {index:0});
        document.getElementById("toolbar-milestoneSubmenuTool").classList.add("active")
        var num2 = 1
        this.milestoneToolbarButton.onClick = function () {
            if(num2%2 ==1){
                //APImodel.esdMilestone()
                console.log(111)
                viewer.ZhiUTech_Milepost.L_SetVisible(false);// 方旋
                document.getElementById("toolbar-milestoneSubmenuTool").classList.remove("active")
            }else{

                viewer.ZhiUTech_Milepost.L_SetVisible(true);// 方旋
                document.getElementById("toolbar-milestoneSubmenuTool").classList.add("active")
            }
            num2++
        }
...
(function(){ 'use strict';
    ...
    function MeasureToolIndicator( viewer, measurement, measureTool )
    {
        ...
        this.endpoints = [];
        this.lines = {
            xyz: {axis: false, material: createLineMaterial('005BCE'), className: 'zu-icon-axis-delta' + '-xyz measure-label-axis-delta' + ' measure-label-axis-xyz'},
            x:   {axis: true,  material: createLineMaterial('F12C2C'), className: 'zu-icon-axis-delta' + '-x measure-label-axis-delta' + ' measure-label-axis-x', iconText: 'X'},
            y:   {axis: true,  material: createLineMaterial('0BB80B'), className: 'zu-icon-axis-delta' + '-y measure-label-axis-delta' + ' measure-label-axis-y', iconText: 'Y'},
            z:   {axis: true,  material: createLineMaterial('2C2CF1'), className: 'zu-icon-axis-delta' + '-z measure-label-axis-delta' + ' measure-label-axis-z', iconText: 'Z'}
        };
        this.segments = [];
        this.dashedLine = {};
        this.simple = false;
...
(function(){ 'use strict';
    ...
    proto.init = function() {
        ...
        //111 lfx
        this.markupToolbar = new avu.ControlGroup(ZhiUTech.Viewing.TOOLBAR.MARKUPTOOLSID);
        toolbar.addControl(this.markupToolbar, toolbarOptions);

        function diffType(type){
            if(window.isEnterEditMode == false){
                markupCore.enterEditMode();
                window.isEnterEditMode = true
            }
            var className = type;
            var editMode = new ZhiUTech.Viewing.Extensions.Markups.Core[className](window.markupCore);
            window.markupCore.changeEditMode(editMode);
            var currentView = window.viewer.autocam.getCurrentView();
            window.currentView=currentView;
            var currenntDiv = document.getElementById("toolbar-markupTool-"+type)
            currenntDiv.classList.add("active");
            var sib=siblingElems(currenntDiv);  //获得相邻元素数组
            for(var i=0;i<sib.length;i++){
                sib[i].classList.remove("active");
            }
            $("#newBoxPan").css({display: "none"});
            markupCore.setStyle(window.editModeColor);
        }
        function diffTypeWeight(type){
            if(window.isEnterEditMode == false){
                markupCore.enterEditMode();
                window.isEnterEditMode = true
            }
            ...
            markupCore.setStyle(window.editModeColor);
            markupCore.setStyle(style);
        }
        $("#markupTools").append("<div id='newBoxPan'><div class='zu-button-icon icon-bi iconfont'></div></div>")
        // 画笔
        this.markupBrushBtn = new avu.Button("toolbar-markupTool-EditModeFreehand");
        this.markupBrushBtn.setToolTip("Markup Brush");
        this.markupBrushBtn.setIcon("icon-pan_icon");//
        this.markupBrushBtn.onClick = function () {
            diffType('EditModeFreehand');
            $("#newBoxPan").css({display: "block"});
            $("#newBoxPan").click(function(){
                diffTypeWeight('EditModeFreehand');
            });
        }
        this.markupToolbar.addControl(this.markupBrushBtn);


        // 箭头
        this.markupStartBtn = new avu.Button("toolbar-markupTool-EditModeArrow");
        this.markupStartBtn.setToolTip("Markup Arrow");
        this.markupStartBtn.setIcon("icon-ai37");
        this.markupStartBtn.onClick = function () {
            diffType('EditModeArrow');
        }
        this.markupToolbar.addControl(this.markupStartBtn);


        // 线条
        this.markupLineBtn = new avu.Button("toolbar-markupTool-EditModePolyline");
        this.markupLineBtn.setToolTip("Markup Line");
        this.markupLineBtn.setIcon("icon-xiantiao");
        this.markupLineBtn.onClick = function () {
            diffType('EditModePolyline');
        }
        this.markupToolbar.addControl(this.markupLineBtn);


        // 矩形
        this.markupRectangularBtn = new avu.Button("toolbar-markupTool-EditModeRectangle");
        this.markupRectangularBtn.setToolTip("Markup Rectangular");
        this.markupRectangularBtn.setIcon("icon-xingzhuang-juxing");
        this.markupRectangularBtn.onClick = function () {
            diffType('EditModeRectangle');
        }
        this.markupToolbar.addControl(this.markupRectangularBtn);

        // 圆形
        this.markupCircularBtn = new avu.Button("toolbar-markupTool-EditModeCircle");
        this.markupCircularBtn.setToolTip("Markup Circular");
        this.markupCircularBtn.setIcon("icon-yuanxingweixuanzhong");
        this.markupCircularBtn.onClick = function () {
            diffType('EditModeCircle');
        }
        this.markupToolbar.addControl(this.markupCircularBtn);


        // 文字
        this.markupTextBtn = new avu.Button("toolbar-markupTool-EditModeText");
        this.markupTextBtn.setToolTip("Markup Text");
        this.markupTextBtn.setIcon("icon-wenzi");
        this.markupTextBtn.onClick = function () {
            diffType('EditModeText');
        }
        this.markupToolbar.addControl(this.markupTextBtn);

        //颜色
        this.markupColorBtn = new avu.Button("toolbar-markupTool-EditModeColor");
        this.markupColorBtn.setToolTip("Markup Color");
        this.markupColorBtn.setIcon("myEditColor");
        this.markupColorBtn.onClick = function () {
        }
        this.markupToolbar.addControl(this.markupColorBtn);
        ...
        //撤销上一步
        this.markupUndoBtn = new avu.Button("toolbar-markupTool-EditModeCircle1");
        this.markupUndoBtn.setToolTip("Markup Undo");
        this.markupUndoBtn.setIcon("icon-chexiaofanhuichehuishangyibu");
        this.markupUndoBtn.onClick = function () {
            markupCore.undo()
        }
        this.markupToolbar.addControl(this.markupUndoBtn);

        //撤销全部
        this.markupUndoAllBtn = new avu.Button("toolbar-markupTool-EditModeCircle2");
        this.markupUndoAllBtn.setToolTip("Markup Undo All");
        this.markupUndoAllBtn.setIcon("icon-zhongzuoredo5");
        this.markupUndoAllBtn.onClick = function () {
            markupCore.clear()
        }
        this.markupToolbar.addControl(this.markupUndoAllBtn);


                //保存绘制内容
                this.markupSaveBtn = new avu.Button("toolbar-markupTool-Save");
                var cancelText1 ='保存'
                this.markupSaveBtn.setToolTip(cancelText1);
                var cancelLabel1 = document.createElement('label');
                cancelLabel1.textContent = cancelText1;
                var btnContainer1 = this.markupSaveBtn.container;
                btnContainer1.appendChild(cancelLabel1);
                btnContainer1.classList.add('zu-label-button');
                var iconEle1 = btnContainer1.getElementsByClassName('zu-button-icon');
                iconEle1 && iconEle1[0] && (iconEle1[0].style.display = 'none');
                this.markupSaveBtn.onClick = function() {
                    var svgstr = markupCore.generateData();
                    var viewstr = window.viewer.autocam.getCurrentView();
                    window.svgstr = svgstr;
                    window.viewstr = JSON.stringify(viewstr);
                    console.log(window.viewstr)

                }.bind(this);
                this.markupToolbar.addControl(this.markupSaveBtn);

        /*
                        //获取绘制内容
                        this.markupGetBtn = new avu.Button("toolbar-markupTool-Get");
                        var cancelText2 ='获取'
                        this.markupGetBtn.setToolTip(cancelText2);
                        var cancelLabel2 = document.createElement('label');
                        cancelLabel2.textContent = cancelText2;
                        var btnContainer2 = this.markupGetBtn.container;
                        btnContainer2.appendChild(cancelLabel2);
                        btnContainer2.classList.add('zu-label-button');
                        var iconEle2 = btnContainer2.getElementsByClassName('zu-button-icon');
                        iconEle2 && iconEle2[0] && (iconEle2[0].style.display = 'none');
                        this.markupGetBtn.onClick = function() {
                            markupCore.show();
                            var view=JSON.parse(window.viewstr);
                            ...
                            window.viewer.autocam.goToView(newView);
                            markupCore.loadMarkups(svgstr, "mylayer");
                        }.bind(this);
                        this.markupToolbar.addControl(this.markupGetBtn);
                */


        //取消标记
        this.markupCancelBtn = new avu.Button("toolbar-markupTool-Cancel");
        var cancelText ='退出标记'
        this.markupCancelBtn.setToolTip(cancelText);
        var cancelLabel = document.createElement('label');
        cancelLabel.textContent = cancelText;
        var btnContainer = this.markupCancelBtn.container;
        ...
        this.markupCancelBtn.onClick = function() {
            window.viewer.toolController.deactivateTool("markups.core");
            markupCore.leaveEditMode();
            markupCore.hide()
            this.measureExtension.exitMarkupmentMode(this.markupToolbar);
            $("#markupTools").children(".active")[0].classList.remove("active");
        }.bind(this);
        this.markupToolbar.addControl(this.markupCancelBtn);
        ...
        this.measureExtension.measureTool.setUnits('mm');
        window.measureExtension =this.measureExtension
        ...
        this.markupToolbar.setVisible(false);


        // markup start
        this.markupToolbar = new avu.ControlGroup(ZhiUTech.Viewing.TOOLBAR.MARKUPTOOLSID);
        // Create a button for the markup
        this.measureSimpleDistanceBtn1 = new avu.Button("toolbar-measureTool-simple-distance1");
        this.measureSimpleDistanceBtn1.setToolTip("Distance1");
        this.measureSimpleDistanceBtn1.setIcon("zu-icon-measure-distance-new");
        this.measureSimpleDistanceBtn1.onClick = function() {

        };

        this.markupToolbar.addControl(this.measureSimpleDistanceBtn1);
