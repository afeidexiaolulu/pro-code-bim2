 ...
 *  功能说明 : 设置模块
 */
function Initialize_ZhiUTech_Settings(core) {
    let mgr={};
    // region 成员
    let _member={};
    _member._qualityFirst=true;// 质量优先
    _member._instantRendering=true;// 即时渲染
    _member._FXAA=true;// 抗锯齿
    _member._groundShadow=true;// 地面阴影
    _member._groundReflection=true;// 地面反射
    _member._shadow=false;// 阴影
    _member._shadowPosition=new THREE.Vector3(0,0,0);// 阴影方向
    _member._sequentialRendering=true;// 顺序渲染

    _member._specialEffectFiter=true;// 特效滤镜
    _member._colorFidelity=true;// 颜色保护
    _member._colorEnhancement=true;// 色彩增强
    _member._brightness=0;// 亮度
    _member._contrast=0;// 对比度
    _member._grayscale=0;// 灰度
    _member._exposure=0;// 曝光值

    // _member._skyColor=new THREE.Color(0,170,255);// 天空颜色
    // _member._groundColor=new THREE.Color(0,170,255);// 地面颜色
    _member._skyColor=new THREE.Color(198,229,255);// 天空颜色
    _member._groundColor=new THREE.Color(255,255,255);// 地面颜色
    mgr._member=_member;
    // endregion
    // region 内部方法
    mgr._RefreshCanvas=function() {
        core.viewer.impl.invalidate(true);
    };
    function _ToggleQualityFirst(isOpen) {
        core.viewer.impl.setOptimizeNavigation(!isOpen);
        _member._qualityFirst=isOpen;
    }
    function _ToggleInstantRendering(isOpen) {
        core.viewer.impl.toggleProgressive(!isOpen);
        _member._instantRendering=isOpen;
    }
    function _ToggleFXAA(isOpen) {
        core.viewer.impl.togglePostProcess(false, isOpen);
        _member._FXAA=isOpen;
    }
    function _ToggleGroundShadow(isOpen) {
        core.viewer.impl.toggleGroundShadow(isOpen);
        _member._groundShadow=isOpen;
    }
    function _ToggleGroundReflection(isOpen) {
        core.viewer.impl.toggleGroundReflection(isOpen);
        _member._groundReflection=isOpen;
    }
    function _ToggleShadow(isOpen) {
        core.viewer.impl.toggleShadows(isOpen);
    ...
    function _SetShadowPosition(x,y,z) {
        ...
        core.viewer.impl.setShadowLightDirection(_member._shadowPosition);
    }
    function _ToggleSequentialRendering(isOpen) {
        core.viewer.impl.toggleVizBuffer(!isOpen);
    ...
    function _ToggleSpecialEffectFilter(isOpen){
        ...
        if(isOpen){
            core.viewer.impl.renderer().setPostProcessParameter("style","edging");
            core.viewer.impl.renderer().setPostProcessParameter("edges");
            core.viewer.impl.setLightPreset(5);
            core.viewer.setBackgroundColor(201,253,244,255,252,225);
        }else{
            core.viewer.impl.renderer().setPostProcessParameter("style");
            core.viewer.impl.setLightPreset(3);
            core.viewer.setBackgroundColor(128,128,128,128,128,128);
        }
    }
    function _ToggleColorFidelity_SpecialEffectFilter(isOpen){
        _member._colorFidelity=isOpen;
        if(isOpen){
            core.viewer.impl.renderer().setPostProcessParameter("preserveColor",1);
        }else{
            core.viewer.impl.renderer().setPostProcessParameter("preserveColor");
        }
        mgr._RefreshCanvas();
    }
    function _ToggleColorEnhancement_SpecialEffectFilter(isOpen){
        _member._colorEnhancement=isOpen;
        let mats=core.viewer.impl.matman()._materials;
        if(isOpen){
    ...
    function _SetBrightness_SpecialEffectFilter(value){
        ...
        core.viewer.impl.renderer().setPostProcessParameter("brightness",value);
        mgr._RefreshCanvas();
    }
    function _SetContrast_SpecialEffectFilter(value){
        _member._contrast=value;
        core.viewer.impl.renderer().setPostProcessParameter("contrast",value);
        mgr._RefreshCanvas();
    }
    function _SetGrayscale_SpecialEffectFilter(value){
        _member._grayscale=value;
        core.viewer.impl.renderer().setPostProcessParameter("grayscale",value);
        mgr._RefreshCanvas();
    }
    function _SetExposure_SpecialEffectFilter(value){
        _member._exposure=value;
        core.viewer.impl.setTonemapExposureBias(value);
        mgr._RefreshCanvas();
    }
    ...
    function _SetSkyOrGroundColor(skyR,skyG,skyB,groundR,groundG,groundB) {
        ...
        core.viewer.setBackgroundColor(
    ...
    /**
     * 即时渲染(如果不开启就分批进行渲染显示)
     * @function L_ToggleInstantRendering
     * @param {boolean} isOpen 是否开启
     */
    ...
    /**
     * 地面反射(简单的反射效果)
     * @function L_ToggleGroundReflection
     * @param {boolean} isOpen 是否开启
     */
     ...
     * @function L_ToggleSequentialRendering
     * @param {boolean} isOpen 是否开启
     */
     ...
     * @function L_ToggleSpecialEffectFilter
     * @param {boolean} isOpen 是否开启
     */
    ...
    /**
     * 颜色保真(增强真实度)
     * @function L_ToggleColorFidelity_SpecialEffectFilter
     * @param {boolean} isOpen 是否开启
     */
    ...
    /**
     * 材质增强(颜色鲜明)
     * @function L_ToggleColorEnhancement_SpecialEffectFilter
     * @param {boolean} isOpen 是否开启
     */
     ...
     * @function L_SetBrightness_SpecialEffectFilter
     * @param {float} value 值
     */
     ...
     * @function L_SetContrast_SpecialEffectFilter
     * @param {float} value 值
     */
     ...
     * @function L_SetGrayscale_SpecialEffectFilter
     * @param {float} value 值
     */
     ...
     * @function L_SetExposure_SpecialEffectFilter
     * @param {float} value 值
     */
    ...
    // region 天空颜色
    /**
     * 天空地面颜色(整体色彩值调整 0~255 step:1)
     * @function L_SetSkyOrGroundColor
     */
    ...
    mgr.GetMaterial=function (dbid) {
        ...
        var fragList = core.viewer.model.getFragmentList();

        var fragIds = [];

        core.viewer.model.getData().instanceTree.enumNodeFragments(
            dbid, (fragId) => {
                fragIds.push(fragId)
            });
    ...
    (function () {
        ...
        div.appendChild(_MakeToggle("顺序渲染",isCheck=>{
            mgr.L_ToggleSequentialRendering(true);
        ...
        div.appendChild(_MakeToggle("特效滤镜",isCheck=>{
            mgr.L_ToggleSpecialEffectFilter(true);
        }));
        div.appendChild(_MakeToggle("颜色保真",isCheck=>{
            mgr.L_ToggleColorFidelity_SpecialEffectFilter(true);
        }));
        div.appendChild(_MakeToggle("材质增强",isCheck=>{
            mgr.L_ToggleColorEnhancement_SpecialEffectFilter(true);
        }));
        ...
        div.appendChild(_MakeColorSlider("天空颜色-红",value=>{
            //mgr.L_SetSkyOrGroundColor(parseFloat(value),undefined,undefined, undefined,undefined,undefined);
            mgr.L_SetSkyOrGroundColor(198,undefined,undefined, undefined,undefined,undefined);
            _ChangeExampleColor("_skyColor_Example",_member._skyColor);
        }));
        div.appendChild(_MakeColorSlider("天空颜色-绿",value=>{
            mgr.L_SetSkyOrGroundColor(undefined,229,undefined,
                undefined,undefined,undefined);
            _ChangeExampleColor("_skyColor_Example",_member._skyColor);
        }));
        div.appendChild(_MakeColorSlider("天空颜色-蓝",value=>{
            mgr.L_SetSkyOrGroundColor(undefined,undefined,255,
                undefined,undefined,undefined);
            _ChangeExampleColor("_skyColor_Example",_member._skyColor);
        }));
